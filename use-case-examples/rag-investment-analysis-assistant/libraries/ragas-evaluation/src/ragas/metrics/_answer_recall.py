...
class AnswerRecallStatements(BaseModel):
...
# examples
example_input_1 = AnswerRecallStatements(
...
class LongFormAnswerPrompt(PydanticPrompt[AnswerRecallStatements, SentencesSimplified]):
    ...
    input_model = AnswerRecallStatements
    output_model = SentencesSimplified
    examples = [(example_input_1, example_output_1)]


class StatementAnswerRecallAnswer(BaseModel):
...
class NLIStatementOutput(BaseModel):
    statements: t.List[StatementAnswerRecallAnswer]
...
@dataclass
class AnswerRecall(MetricWithLLM, SingleTurnMetric):
    ...
    async def _create_statements(
        ...
        text, question = row["reference"], row["user_input"]
