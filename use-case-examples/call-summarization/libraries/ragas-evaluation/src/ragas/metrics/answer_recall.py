...
@dataclass
class AnswerRecall(MetricWithLLM):

    """
    Measures answer recall as faithfulness of ground truth against answer
    ...
    def _score_batch(
        ...
        ds_faithfulness = ds_faithfulness.rename_columns({"answer": "contexts"})
        ds_faithfulness = ds_faithfulness.rename_columns({"ground_truths": "answer"})
        df_faithfulness = ds_faithfulness.to_pandas()
        df_faithfulness["contexts"] = df_faithfulness["contexts"].apply(lambda x: [x])
        df_faithfulness["answer"] = df_faithfulness["answer"].apply(
            lambda x: "\n".join(x)
        )
        ds_faithfulness = Dataset.from_pandas(df_faithfulness)
